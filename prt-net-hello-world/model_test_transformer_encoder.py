...
class Model(nn.Module):
  ...
  def init_optim(self):
    ...
    should_update = chain(self.decoder.parameters(), self.query_from_dh_layer.parameters())
    self.optim = optim.SGD(should_update, lr=0.01, momentum=0.9)

  def _inpt_for_encoder(self, nums):
    default_batch_size = 1 
    return self.embedding(t.LongTensor(nums)).view(-1, default_batch_size, self.input_size)
  ...
  def decode_and_train(self, dh, dc, inpt, correct_index, for_select):
    ...
    # print(f'for_softmax: {for_softmax}, correct_index = {correct_index}')
